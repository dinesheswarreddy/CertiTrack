...
function StudentDashboard() {
  ...
  const fetchCertificates = async () => {
    const res = await axios.get(`${API}/api/certificates/mine`, { headers });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`${API}/api/certificates/${editId}`, payload, { headers });
      } else {
        await axios.post(`${API}/api/certificates`, payload, { headers });
      }
...
const handleDelete = async (id) => {
  if (window.confirm('Are you sure you want to delete this certificate?')) {
    await axios.delete(`${API}/api/certificates/${id}`, { headers });
    fetchCertificates();
  }
};
  const resetForm = () => {
  ...
  return (
    <div className="ct-page ct-role-student min-vh-100">
      <Navbar bg="dark" variant="dark" className="ct-topbar">
        <Container className="d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center gap-2">
            <span className="ct-seal ct-seal-sm"><i className="bi bi-mortarboard-fill" /></span>
            <h2 className="mb-0 fs-4"><span className="fw-bold">Welcome, {studentName}</span></h2>
          </div>
          <div>
...
<Container className="mt-4">
  <div className="ct-eyebrow text-center mb-1">Your Records</div>
  <h4 className="text-center mb-3">Internship Completion Certificates</h4>
  {certList.length === 0 ? (
